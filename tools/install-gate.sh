...
if [ -d "$HOOKDIR" ]; then
  ...
  if [ -n "$_vg_hd_top" ] && [ "$_vg_hd_top" -ef "$ROOT" ] \
    ...
    echo "[verax-gate] ERROR: active pre-push hook directory is tracked by this repo (branch-controlled)."
...
if [ -e "$HOOK" ]; then
  while IFS= read -r _vg_tf; do
    ...
    if [ "$HOOK" -ef "$ROOT/$_vg_tf" ]; then
      echo "[verax-gate] ERROR: active pre-push hook resolves to a tracked file ($_vg_tf) — tracked by this repo (branch-controlled)."
      echo "[verax-gate]   Refusing to install the verifier into branch-controlled hook content."
      echo "[verax-gate]   Set core.hooksPath to an untracked hook directory (for example .git/hooks or .husky/_), then re-run this script."
      exit 1
    fi
