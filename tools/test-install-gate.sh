...
make_repo() {
  ...
  if [ "$with_spec" = yes ]; then
    mkdir -p .verax
    cp "$ROOT/.verax/gate.json" .verax/gate.json
  fi
  git add tools/install-gate.sh tools/test-install-gate.sh .githooks/pre-push
  [ "$with_spec" = yes ] && git add .verax/gate.json
  git commit -m init -q
}
...
assert_contains .git/hooks/pre-push '# >>> verax local gate >>>'
...
assert_contains .git/verax-gate.pin 'gate.json absent'
mkdir -p .verax
...
! grep -qF '# >>> verax local gate >>>' .githooks/pre-push || { echo '[install-gate-test] tracked hook was modified'; exit 15; }
...
if [ -d .GITHOOKS ]; then
  ...
  ! grep -qF '# >>> verax local gate >>>' .githooks/pre-push || { echo '[install-gate-test] case-folded tracked hook was modified'; exit 27; }
...
assert_contains custom-hooks/pre-push '# >>> verax local gate >>>'
...
assert_contains .husky/_/pre-push '# >>> verax local gate >>>'
